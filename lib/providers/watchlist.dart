...
class WatchListProvider extends ChangeNotifier {
  ...
  Future<List<Entry>> list() async {
    ...
    final watchlist = await ApiClient.database.listDocuments(
      collectionId: _collectionId, databaseId: 'defaullt',
    );
    ...
    final movieIds = watchlist.documents
        ...
        .toList();
    final entries =
        (await ApiClient.database.listDocuments(collectionId: 'movies', databaseId: 'default'))
            .documents
  ...
  Future<void> remove(Entry entry) async {
    ...
    await ApiClient.database
        .deleteDocument(collectionId: _collectionId, documentId: id, databaseId: 'default');

    list();
  }

  Future<Uint8List> imageFor(Entry entry) async {
    return await ApiClient.storage.getFileView(fileId: entry.thumbnailImageId, bucketId: 'posters');
  }
